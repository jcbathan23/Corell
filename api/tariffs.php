...
switch ($method) {
	...
	case 'POST':
		...
		$stmt = $mysqli->prepare('INSERT INTO tariffs (name, category, base_rate, per_km_rate, per_hour_rate, priority_multiplier, status, effective_date, expiry_date, notes) VALUES (?,?,?,?,?,?,?,?,?,?)');
		$stmt->bind_param(
			'ssddddssss',
			$body['name'],
			$body['category'],
...
function after_write_response(int $id): void {
	global $mysqli;
	$get = $mysqli->prepare('SELECT * FROM tariffs WHERE id = ?');
	$get->bind_param('i', $id);
	$get->execute();
